        ...
        for iter in range(N_iters):
            ...
            for i in range(N):
                ...
                for j in range(1, len(X[i]) + 1):
                    ...
                    # marg2_ij is a length-L vector of those probabilities with
                    # each element corresponding to each state
                    # Divide the vector by the sum of the elems (alpha * beta)
                    ...
                    # If j is the last index, j + 1 is out of bounds and
                    # marg3_ij cannot be computed
                    if j == len(X[i]):
                        break

                    # Compute marg3_ij_prob for each state a and b
                    # marg3_ij is a L x L matrix of those probabilities with
                    # each element corresponding to a pair of states a, b
                    marg3_ij = [[0. for _ in range(self.L)]
